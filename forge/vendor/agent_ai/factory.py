...
from forge.vendor.agent_ai.providers.base import ProviderClient

if TYPE_CHECKING:
    from forge.vendor.agent_ai.client import AgentAIConfig
...
def build_provider_client(config: "AgentAIConfig") -> ProviderClient:
    ...
    if config.provider == "claude":
        from forge.vendor.agent_ai.providers.claude import ClaudeProviderClient, ClaudeProviderConfig
    ...
    if config.provider == "codex":
        from forge.vendor.agent_ai.providers.codex import CodexProviderClient, CodexProviderConfig
    ...
    if config.provider == "opencode":
        from forge.vendor.agent_ai.providers.opencode import (
    ...
    if config.provider == "openrouter_direct":
        from forge.vendor.agent_ai.providers.openrouter_direct import (
            OpenrouterDirectClient,
            OpenrouterDirectConfig,
        )

        provider_cfg = OpenrouterDirectConfig(
            model=config.model,
            system_prompt=config.system_prompt,
            max_retries=config.max_retries,
            initial_delay=config.initial_delay,

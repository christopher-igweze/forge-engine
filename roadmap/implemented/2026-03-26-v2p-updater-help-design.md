...
vibe2prod update --check  # dry run — detect changes but don't apply them
...
Migration functions live in `forge/migrations.py`. Each migration is a function:

```python
def migrate_1_to_2(config: dict) -> dict:
    """Add codebase_guide and test_audit sections with defaults."""
    config.setdefault("codebase_guide", {"depth": "standard", "auto_generate": True})
    config.setdefault("test_audit", {"auto_generate": True, "realtime_feedback": True})
    config["config_schema_version"] = 2
    return config
```

The updater discovers migrations by matching manifest's `migrations` keys (e.g., `"1->2"`) to functions named `migrate_X_to_Y`. Migrations run sequentially: if current schema is 1 and target is 3, it runs `1->2` then `2->3`.
...
**When already up to date:** If all steps report "unchanged", print a single line: `Already up to date (v1.2.0)` instead of listing all 5 steps.

**`--check` mode:** Runs all detection logic (hashing, config reading, version comparison) but skips all mutations (pip install, file writes, MCP registration). Output shows what *would* change.
...
- **Suppresses repeated notifications** — after printing once, writes a `last_notified` timestamp to the cache. Suppresses the notification for 1 hour to avoid repeating on consecutive commands.
- **Respects config**: `vibe2prod config set auto_update_check false` disables it. (Prerequisite: add `auto_update_check` to `KNOWN_KEYS` in `config_io.py` as a boolean validator.)
- **No network call** if cache is fresh (within TTL).
- **No check during `vibe2prod update`** — it already checks PyPI as part of Step 1.

### Implementation

Add a `check_for_update()` function called in a new Typer app callback (`@app.callback(invoke_without_command=True)` — must preserve existing `no_args_is_help` behavior). Uses `threading.Thread(daemon=True)` to avoid blocking. Writes cache atomically (same pattern as `config_io.py`). Cache file uses default permissions (no secrets — only version strings and timestamps).
...
AUTHENTICATION
  auth <action>        Log in, log out, or check auth status (login/logout/status)

> Note: `auth` is currently a single command with a positional action argument, not a sub-app.
> As a prerequisite, convert `auth` to a Typer sub-app (like `config_app`) so subcommands
> render correctly in help output.
...
`help` is a new Typer command. Note: Typer allows registering a command named `help` without conflicting with `--help` (tested — they use different code paths). It reads command metadata from a registry (dict mapping command names to their help content). The help content is defined alongside each command, not in a separate file, so it stays in sync.
...
`forge/manifest.json` is generated by `scripts/build_manifest.py` and **checked into git** (not gitignored). This ensures the manifest is always in sync with the packaged release.

**Generation script (`scripts/build_manifest.py`):**
- Reads version from `pyproject.toml`
- Computes SHA-256 hashes of all `forge/skills/*/SKILL.md` files
- Reads hook/MCP definitions from a static registry dict in the script
- Config schema version incremented manually when schema changes
- Writes `forge/manifest.json`

**When to run:** Before each release (`python scripts/build_manifest.py`). CI can enforce that the manifest is up to date by comparing generated vs committed.

**Prerequisite:** Fix `pyproject.toml` package-data to include all skills: change to `forge = ["skills/*/SKILL.md"]` so both `forge` and `forgeignore` skills are packaged.

## Prerequisites

These must be done before implementing the update pipeline:

1. **Fix version mismatch** (see below) — auto-check and update both depend on accurate version detection
2. **Fix `pyproject.toml` package-data** — include all skills: `forge = ["skills/*/SKILL.md"]`
3. **Add `auto_update_check` to `KNOWN_KEYS`** in `config_io.py`
4. **Convert `auth` to Typer sub-app** — currently a single command with positional action arg, needs to be a sub-app for help grouping
...
| Flag | Action |
|------|--------|
| `--max-cost` | Deprecate in next release (print warning), remove in following release |
| `--max-time` | Same — deprecate then remove |
...
| `pip` not available | Try `pipx`, then `pip3`, then `pip` (matches `install.sh` preference order). If all fail, report error with manual install instructions |
